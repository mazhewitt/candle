...
fn main() -> Result<()> {
    ...
    let model_filename = match &args.model_file {
        ...
        None => {
            ...
            if model_file.extension().and_then(|s| s.to_str()) == Some("mlmodel") {
                ...
                let compiled_model_path =
                    cache_dir.join("compiled_models").join(&compiled_model_name);
                ...
                if !compiled_model_path.exists() {
                    ...
                    let source_path = if mlpackage_dir.extension().and_then(|s| s.to_str())
                        == Some("mlpackage")
                    {
                        mlpackage_dir
                    } else {
    ...
    let tokenizer_filename = match &args.tokenizer_file {
        ...
        None => {
            ...
            let tokenizer_patterns = [("tokenizer.json", &api), ("tokenizer_config.json", &api)];
